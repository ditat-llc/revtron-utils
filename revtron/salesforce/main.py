...
class Salesforce:
    ...
    def _upsert(
        ...
        **kwargs
        ...
        response = {'status': 'success', 'method': method}
        if Id is not None:
            url += f'/{Id}'
            response['Id'] = Id
        try:
            api_resp = self.request(method, url=url, json=kwargs)
            if 'id' in api_resp:
                response['Id'] = api_resp['id']
    ...
    def update(self, sobject: str, records: dict | list[dict]) -> list[dict] | dict:
        return self._bulk_upsert(sobject, records)

    def insert(self, sobject: str, records: dict | list[dict]) -> list[dict] | dict:
        return self._bulk_upsert(sobject, records)

    def upsert(self, sobject: str, records: dict | list[dict]) -> list[dict] | dict:
        return self._bulk_upsert(sobject, records)
